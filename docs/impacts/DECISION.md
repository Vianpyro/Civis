...
| R-8 | ~~Le fichier `docs/impacts/DECISION.md` n'est pas suivi par git : `.gitignore` ignore `*.md`~~ | Sans objet : `.gitignore` ne porte aucune règle `*.md`. Seul `docs/scratch/` est exclu, et ce document est suivi par git comme tout `docs/` | **Clos le 2 août 2026, à la livraison de PR-19.** Le risque était faux à la rédaction ou l'est devenu ; le vérifier coûte une commande (`git ls-files docs`). La ligne est conservée barrée plutôt que supprimée : la numérotation des risques est citée ailleurs et ne se renumérote pas |
